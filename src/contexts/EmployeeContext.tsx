...
export function EmployeeProvider({ children }: { children: ReactNode }) {
  ...
  const getEmployeeIdFromToken = (): string | null => {
    try {
      ...
      return payload.sub || payload.employeeId || payload.userId || payload.id || null;
    } catch {
      return null;
    }
  };
  ...
  const loadEmployee = async () => {
    ...
    let employeeId = getEmployeeIdFromToken() || localStorage.getItem("employeeId");
    ...
    setError(null);
    try {
      const employeeInfo = await fetchEmployeeInfo(employeeId, schoolYear.schoolYearId);
  ...
  return (
    <EmployeeContext.Provider value={{ employee, isLoading, error, refresh: loadEmployee }}>
      {children}
